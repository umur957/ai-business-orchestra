#!/usr/bin/env python3
"""
HaruPlate Orchestra Main Execution
Updated main file that uses the improved HaruPlate Orchestra Flow
"""
...
try:
    from src.flows.haruplate_orchestra_flow_simple import (
        HaruPlateOrchestraFlow, 
        process_haruplate_request,
        create_haruplate_flow
    )
    FLOW_AVAILABLE = True
...
class HaruPlateOrchestra:
    """
    Main HaruPlate Orchestra class
    Enhanced version with proper flow integration
    """
    
    def __init__(self):
        self.company_name = os.getenv("COMPANY_NAME", "HaruPlate")
    ...
    def process_request(self, request: str, user: str = "haruplate_manager") -> Dict[str, Any]:
        """
        Process a business request through the HaruPlate Orchestra
        """
        print(f"\nHaruPlate Orchestra - Processing Request")
        print(f" User: {user}")
        print(f" Request: {request}")
        print("-" * 60)
        
        if FLOW_AVAILABLE:
            return process_haruplate_request(request, user)
        else:
    ...
    def _simulate_response(self, request: str) -> Dict[str, Any]:
        ...
        if any(keyword in request_lower for keyword in hr_keywords):
            request_type = "hr"
            simulation_result = f"""
HaruPlate Orchestra - HR Expert Team Response

 Request: {request}

 HR EXPERT TEAM ANALYSIS:
Our HR team has analyzed your request with HaruPlate's family-oriented values:
...
NEXT STEPS:
1. Create HaruPlate-aligned job description (2-3 hours)
...
All processes maintain HaruPlate's warm, health-focused brand voice.
"""
        else:
            request_type = "admin"
            simulation_result = f"""
HaruPlate Orchestra - Admin Expert Team Response
...
• Standardized HaruPlate documentation
• Enhanced financial data organization
• Better compliance tracking

All operations align with HaruPlate's operational excellence.
"""
    ...
    def run_scenario(self, scenario_type: str, context: str) -> Dict[str, Any]:
        """Run a specific HaruPlate business scenario"""
        ...
        scenarios = {
            ...
            "daily_operations": "Coordinate team meeting for 15 HaruPlate staff with catering that aligns with our healthy nutrition values and presentation setup for product roadmap."
        }
        ...
        if scenario_type in scenarios:
            ...
            print(f"\nRunning HaruPlate Scenario: {scenario_type.replace('_', ' ').title()}")
...
def main():
    """Main function demonstrating HaruPlate Orchestra capabilities"""
    
    print("HaruPlate Orchestra - Intelligent HR & Admin Automation")
    print("=" * 65)
    print("Like HaruPlate's founders, focus on strategy & product vision")
    print("AI Let the Orchestra handle repetitive HR & admin tasks")
    print("=" * 65)
    
    # Initialize HaruPlate Orchestra
    ...
    # Demo requests with HaruPlate context
    demo_requests = [
    ...
    print(f"\n Testing HaruPlate Orchestra with Demo Requests...")
    print("-" * 65)
    ...
    for i, (request_type, request) in enumerate(demo_requests, 1):
        ...
        print(f"\n Processing through HaruPlate Orchestra...")
        ...
        try:
            ...
            if result.get("error"):
                ...
                print(f"HaruPlate Values Applied: {'YES' if result.get('haruplate_values_applied') else 'NO'}")
    ...
    # Interactive mode
    print(f"\nInteractive Mode:")
    print("Enter your HaruPlate business requests, or 'quit' to exit")
    ...
    print(f"\nThank you for using HaruPlate Orchestra!")
